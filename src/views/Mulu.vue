...
export default{
    ...
    setup(props){
        ...
        window.addEventListener("scroll",function(){
            progress.value=
            parseInt(
                (window.scrollY/this.document.documentElement.scrollHeight)*100
            )+"%";
            let visibleTitles=[]; 
            for(let i=titles.length-1;i>=0;i++){
                const title=titles[i];
                if(title.scrollTop<=window.screenY){
                    if(currrentTitle.id===title.id)
                    retern;
                    Object.assign(currrentTitle,title);
                    //展开节点
